...
Description: "This Observation profile is based on the US Core Simple Observation Profile with requirements added from the IHE SCD eCC Observation profile. It adds category to differentiate Section and Question Observations. Section(s) contain one or more hasMember relationships to Questions, will not have a value element, and have no derivedFrom relationships. Questions have a derivedFrom relationship with Section AND contain EITHER a value element OR one or more hasMember relationships."
...
* category[report-category] from CAPeCCSectionCodes (required)
...
// PATRICK - There is no ValueSet identified. If there is an expection of using CAP codes, there should be a valueset
